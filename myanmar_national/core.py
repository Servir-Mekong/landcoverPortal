...
class MyanmarNational():
    ...
    def __init__(self, area_path, area_name, shape, geom, radius, center):
        ...
        if (area_path and area_name):
            if (area_path == 'country'):
                ...
                self.geometry = MyanmarNational.MEKONG_FEATURE_COLLECTION.filter(\
                ...
                #self.geometry = MyanmarNational.DEFAULT_GEOM.buffer(10000)
                self.geometry = MyanmarNational.DEFAULT_GEOM
        else:
    ...
    def _get_geometry(self, shape):
        ...
        return MyanmarNational.DEFAULT_GEOM
    ...
    def get_landcover(self, classes=range(0, 11), year=2017, download=False):
        ...
        palette = []
        for _class in MyanmarNational.LANDCOVERCLASSES:
        ...
        map_id = image.getMapId({
            'min': '0',
            'max': str(len(MyanmarNational.LANDCOVERCLASSES) - 1),
            'palette': palette
        })
    ...
    def get_stats(self, year=2017, classes=range(0, 12)):
        ...
        return {MyanmarNational.INDEX_CLASS[int(float(k))]:float('{0:.2f}'.format(v)) for k,v  in data.items()}
